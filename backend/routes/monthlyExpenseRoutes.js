// backend/routes/monthlyExpenseRoutes.js
...
function isTemplateApplicable(t, monthYYYYMM) {
  ...
  if (t.isRecurring) {
    ...
    if (t.expiryDate) {
      ...
      return firstMonth && lastMonth && firstMonth <= monthYYYYMM && monthYYYYMM <= lastMonth;
    }
...
router.get('/:month', auth, async (req, res) => {
  ...
  try {
    ...
    for (const t of templates) {
      ...
      const exists = monthlyDoc.templates.some(temp => temp.templateId.equals(t._id));
      if (!exists) {
    ...
    const totals = {
      total: items.reduce((sum, i) => sum + Number(i.amountOverride ?? i.amount), 0),
      recurring: items.filter(i => i.isRecurring).reduce((sum, i) => sum + Number(i.amountOverride ?? i.amount), 0),
      onceOff: items.filter(i => !i.isRecurring).reduce((sum, i) => sum + Number(i.amountOverride ?? i.amount), 0),
    };
...
router.patch('/:id', auth, async (req, res) => {
  try {
    ...
    if (!monthlyDoc) return res.status(404).json({ message: 'Monthly expense not found' });
...
router.post('/copy', auth, async (req, res) => {
  try {
    ...
    if (!fromMonth || !toMonth)
      return res.status(400).json({ message: 'fromMonth and toMonth are required' });

    const prevDoc = await MonthlyExpense.findOne({ user: userId, month: fromMonth });
    if (!prevDoc) return res.status(404).json({ message: 'Previous month not found' });

    let newDoc = await MonthlyExpense.findOne({ user: userId, month: toMonth });
    if (!newDoc) newDoc = new MonthlyExpense({ user: userId, month: toMonth, templates: [] });

    for (const t of prevDoc.templates) {
      const exists = newDoc.templates.some(temp => temp.templateId.equals(t.templateId));
      if (!exists) {
